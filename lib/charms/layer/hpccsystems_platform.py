...
impmort lib.charms.layer.utils
...
class HPCCSystemsPlatformConfigs:
    def __init__(self)
        ...
        self.version = config['hpcc-version']
        self.cluster_node_types=
        self.nodes = {}
        
     def install(self):
         install_prerequisites(self.config('hpcc-type')) 
     ...
     def get_platform_package_name(self):
         ...
         return "systems-platform-" + full_hpcc_type[hpcc_type] + "_" + 
               self.config('hpcc_version')platform.dist[2] + "_amd64.deb"

     def get_platform_package_url(self):
         hpcc_version_mmp = self.hpcc_version.split('-',1)[0]
     ...
     def install_prerequsites(self):
         ...
         charm_dir = hostenv.charm_dir()
         with open(os.path.join(charm_dir,'community.yaml')) as fp:
             workload = yaml.safe_load(fp)
         packages = workload['packages']
         addition_file = ""
         hpcc_type = self.config['hpcc-type']
         if hpcc_type == "EE":
             addition_file = os.path.join(charm_dir, "enterprise.yaml")
         elif hpcc_type == "LN":
             addition_file = os.path.join(charm_dir, "internal.yaml")
      ...
      def start(self):
          ...
          log(HPCC_HOME+'/etc/init.d/hpcc-init start', INFO)
          try: 
               output = check_output([HPCC_HOME+'/etc/init.d/hpcc-init', 'start'])
               log(output, INFO)
      ...
      def stop(self):
          ...
          log(HPCC_HOME+'/etc/init.d/hpcc-init stop', INFO)
          try: 
              output = check_output([HPCC_HOME+'/etc/init.d/hpcc-init', 'stop'])
              log(output, INFO)
          exception CalledProcessError:
               log(output, ERROR)
               return 1
      ...
      def restart(self):
          ...
          log(HPCC_HOME+'/etc/init.d/hpcc-init stop', INFO)
          try: 
              output = check_output([HPCC_HOME+'/etc/init.d/hpcc-init', 'restart'])
              log(output, INFO)
          exception CalledProcessError:
               log(output, ERROR)
               return 1
      ...
      def create_envgen_cmd(self):
          ...
          for key in nodes.key():
              ...
              if nodeType == 'support':
                  with open( CLUSTER_IPS_DIR + '/' + key, 'r') as file:
      ...
      def nodes_per_type(self, nodeType):
          ipFile =  CLUSTER_IPS_DIR + '/' + nodeType
